...
This guide explains how to inspect live Postgres statistics.

You or an agent can run the same checks from:
...
A ratio below 99% means Postgres is reading from disk more than from cache. Treat that as a [Performance](/docs/guides/observability/detecting#performance) signal, then search [Diagnosing](/docs/guides/troubleshooting).
