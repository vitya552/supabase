    ...
    case 'GET':
      if (!IS_MANAGEMENT_API_ENABLED) {
        return res.status(404).json({
          error: { message: 'Storage configuration requires the management API' },
        })
      }
      return proxyManagementApi(
        req,
        res,
        `/platform/projects/${encodeURIComponent(ref)}/config/storage`
      )
    case 'PATCH':
      return res.status(400).json({
        error: {
          message:
            'Storage configuration is read-only in self-hosted deployments; update the storage service environment (.env / docker-compose) instead',
        },
      })
    default:
